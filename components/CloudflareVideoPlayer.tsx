...
import { Play, Lock, AlertCircle, RefreshCw, CheckCircle, Clock, Cloud, ShieldAlert } from "lucide-react-native";
...
export default function CloudflareVideoPlayer({
  ...
  const screenCaptureKey = `cloudflare-video-protection-${videoId}`;

  // CRITICAL: Use the hook to prevent screen capture as long as component is mounted
  // This is the most reliable method - protection starts immediately on mount
  if (Platform.OS !== "web" && !isLocked) {
    // eslint-disable-next-line react-hooks/rules-of-hooks
    usePreventScreenCapture(screenCaptureKey);
  }

  // Screenshot listener - detect and alert when screenshots are taken
  useScreenshotListener(() => {
    if (Platform.OS !== "web" && !isLocked) {
      console.log("[CloudflarePlayer] Screenshot detected! Content is protected.");
      Alert.alert(
        "Screenshot Blocked",
  ...
  useEffect(() => {
    ...
    const enableAppSwitcherProtection = async () => {
      try {
        ...
        console.log("[CloudflarePlayer] iOS app switcher blur protection ENABLED");
      } catch (err) {
        console.log("[CloudflarePlayer] App switcher protection not available:", err);
      }
    };
  ...
  useEffect(() => {
    ...
    const checkProtection = async () => {
      try {
        ...
        if (isAvailable) {
          setIsScreenCaptureProtected(true);
          console.log("[CloudflarePlayer] Screen capture protection is ACTIVE for video:", videoId);
        }
      } catch (err) {
        console.log("[CloudflarePlayer] Error checking screen capture availability:", err);
      }
    };
  ...
  const signedUrlQuery = trpc.cloudflare.getSignedUrl.useQuery(
    { videoId, accountId },
    {
  ...
  const videoInfoQuery = trpc.cloudflare.getVideoInfo.useQuery(
    { videoId, accountId },
    {
      enabled: !isLocked,
      staleTime: 5 * 60 * 1000,
      ...
      >
        ...
        <View style={styles.playOverlay}>
          <View style={styles.providerBadge}>
            <Cloud color={Colors.surface} size={14} />
            <Text style={styles.providerBadgeText}>Cloudflare Stream</Text>
          </View>
      ...
      <View style={styles.playerContainer}>
        <WebView
          ...
          onError={(syntheticEvent) => {
            ...
            console.error("[CloudflarePlayer] WebView error:", nativeEvent);
            setError("Failed to load video. Please try again.");
          }}
        />
      </View>
