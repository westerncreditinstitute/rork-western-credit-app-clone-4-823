...
export default function AIDisputeAssistantScreen() {
  ...
  const handleAccountsParsed = useCallback((accounts: ParsedAccount[], bureau: string) => {
    ...
    const newAccounts: NegativeAccount[] = negativeAccounts.map((acc, index) => ({
      ...
      // Carry the parser's furnisher address through to the dispute
      // flow — without this, the letter-generation step has no way to
      // address the letter to the correct furnisher even when the
      // parser extracted it correctly.
