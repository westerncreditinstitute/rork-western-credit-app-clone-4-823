...
export class BusinessService {
  ...
  static async getUserBusinesses(userId: string): Promise<UserBusinessData[]> {
    try {
      ...
      if (error) {
        ...
        console.log('[BusinessService] Error fetching user businesses:', msg, 'code:', error.code, 'details:', error.details);
        return [];
      }

      return data as UserBusinessData[];
    } catch (error: any) {
      console.log('[BusinessService] Exception in getUserBusinesses:', error?.message || String(error));
      return [];
    }
  }
  ...
  static async getBusinessById(businessId: string): Promise<UserBusinessData | null> {
    try {
      ...
      if (error) {
        console.log('[BusinessService] Error fetching business:', error?.message || error?.code || JSON.stringify(error));
        return null;
      }
  ...
  static async createBusiness(
    ...
    try {
      ...
      if (error) {
        console.log('[BusinessService] Error creating business:', error?.message || error?.code || JSON.stringify(error));
        return {
          success: false,
  ...
  static async updateBusiness(
    ...
    try {
      ...
      if (error) {
        console.log('[BusinessService] Error updating business:', error?.message || error?.code || JSON.stringify(error));
        return {
          success: false,
  ...
  static async deleteBusiness(businessId: string, userId: string): Promise<boolean> {
    try {
      ...
      if (error) {
        console.log('[BusinessService] Error deleting business:', error?.message || error?.code || JSON.stringify(error));
        return false;
      }
