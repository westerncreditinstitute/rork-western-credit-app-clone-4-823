...
export class InvestmentPoolService {
  ...
  static async getOpenPools(): Promise<InvestmentPoolData[]> {
    try {
      ...
      if (error) {
        console.error('[InvestmentPoolService] Error fetching open pools:', error);
        return [];
      }

      return data as InvestmentPoolData[];
    } catch (error) {
      console.error('[InvestmentPoolService] Exception in getOpenPools:', error);
      return [];
    }
  }
  ...
  static async getPoolsByBusiness(businessId: string): Promise<InvestmentPoolData[]> {
    try {
      ...
      if (error) {
        console.error('[InvestmentPoolService] Error fetching business pools:', error);
        return [];
      }

      return data as InvestmentPoolData[];
    } catch (error) {
      console.error('[InvestmentPoolService] Exception in getPoolsByBusiness:', error);
      return [];
    }
  }
  ...
  static async getUserContributions(userId: string): Promise<PoolContributionData[]> {
    try {
      ...
      console.error('[InvestmentPoolService] Exception in getUserContributions:', error);
      return [];
    }
  }
  ...
  static async contributeToPool(
    ...
    try {
      if (!isValidUUID(userId)) {
        console.log('[InvestmentPoolService] Skipping contribution for non-UUID user:', userId);
        return {
          success: false,
